{
  ...
  methods: {
    ...
    custom_input_schema: lambda do |input|
      input_schema =
        ...
        when 'users'
          parse_json(input&.dig('users', 'dataFields', 'input', 'schema') || '[]')
        when 'subscribers'
          parse_json(input&.dig('subscribers', 'dataFields', 'input', 'schema') || '[]')
        when 'catalog_item'
          parse_json(input&.
        ...
        when 'purchase'
          if input['flag'] == 'user'
            parse_json(input['input']&.dig('user', 'dataFields', 'input', 'schema') || '[]')
          elsif input['flag'] == 'items'
            parse_json(input['input']&.dig('items', 'dataFields', 'input', 'schema') || '[]')
          else
            parse_json(input['input']&.dig('dataFields', 'input', 'schema') || '[]')
          end
        else
          parse_json(input&.dig('dataFields', 'input', 'schema') || '[]')
        end
      [
        {
          name: 'input',
      ...
      [
        {
          ...
          hint: 'Use this to define your data fields from the output.',
          extends_schema: true,
          schema_neutral: true,
          sticky: true
        }
    ...
    purchase: lambda do |input|
      [
        { label: 'Purchase ID', name: 'id', sticky: true,
          hint: 'If a purchase exists with the given ID, the purchase will be updated. ' \
            'If none is specified, a new ID will automatically be generated and returned.' },
        { properties: [
          { name: 'email', sticky: true,
            hint: 'Either email or user ID must be passed in to identify the user. ' \
              'If both are passed in, email takes precedence.' },
          { name: 'dataFields', type: 'object', extends_schema: true,
            hint: 'Data fields to store in the user profile.',
            properties: call('custom_input_schema', 'input' => input, 'flag' => 'user', 'object_name' => 'purchase') },
          { name: 'userId', sticky: true,
            hint: 'Optional user ID, typically your database generated ID. ' \
              'Either email or user ID must be specified.' },
          { control_type: 'checkbox',
            label: 'Prefer user ID',
            sticky: true,
            parse_output: 'boolean_conversion',
            render_input: 'boolean_conversion',
            hint: 'Create a new user with the specified user ID if the user does not exist yet.',
          ...
          { control_type: 'checkbox',
            ...
            type: 'boolean',
            name: 'mergeNestedObjects' }
        ], type: 'object', optional: false, name: 'user' },
        {
          name: 'items',
          type: 'array',
          of: 'object',
          ...
          properties: [
            ...
            { label: 'SKU', name: 'sku', sticky: true },
            { name: 'name', optional: false },
            { name: 'description', sticky: true },
            { name: 'categories', sticky: true,
              hint: 'Categories this product belongs to. It can belong to multiple categories.<br>' \
                'Please enter values separated by commas, without spaces.' },
            { control_type: 'number', parse_output: 'float_conversion',
              render_input: 'float_conversion', type: 'number',
              optional: false, name: 'price' },
            { control_type: 'number', parse_output: 'float_conversion',
              render_input: 'float_conversion', type: 'number',
              optional: false, name: 'quantity' },
            { name: 'imageUrl', sticky: true,
              hint: 'URL to an image of the item.' },
            { name: 'url', sticky: true,
              hint: 'URL to the item.' },
            { name: 'dataFields', type: 'object', extends_schema: true,
              hint: 'Additional item properties.',
              properties: call('custom_input_schema', 'input' => input, 'flag' => 'items', 'object_name' => 'purchase') }
          ]
        },
        { control_type: 'integer', parse_output: 'integer_conversion',
          render_input: 'integer_conversion', type: 'integer',
          sticky: true, name: 'campaignId' },
        { control_type: 'integer', type: 'integer',
          parse_output: 'integer_conversion', render_input: 'integer_conversion',
          sticky: true, name: 'templateId' },
        { control_type: 'number', type: 'number',
          parse_output: 'float_conversion', render_input: 'float_conversion',
          optional: false, name: 'total' },
        { sticky: true, control_type: 'date_time',
          hint: 'Time event happened. Set to the time event was received if unspecified.',
          type: 'date_time', name: 'createdAt' }
      ]
    end,
      ...
      [
          ...
          properties: call('custom_input_schema', input) },
        {
          ...
          toggle_field: {
            ...
            optional: false,
            toggle_hint: 'Use custom value',
            hint: 'Please enter catalog name.'
          }
        },
      ...
      when 'purchase'
        input.map do |key, value|
          if %w[dataFields].include?(key)
            ...
            { key => value&.map do |user_key, user_value|
                       if user_key == 'dataFields'
                         { user_key => user_value&.dig('input', 'data') }
                       else
                         { user_key => user_value }
                       end
                     end&.inject(:merge) }
          elsif key == 'items'
            { key => value&.map do |items|
                       items&.map do |item_key, item_value|
                         if item_key == 'dataFields'
                           { item_key => item_value&.dig('input', 'data') }
                         elsif item_key == 'categories'
                           { item_key => item_value.split(',') }
                         else
                           { item_key => item_value }
                         end
                       end&.inject(:merge)
    ...
    upsert_event_schema: lambda do |input|
      call('event', '').
    ...
    upsert_purchase_schema: lambda do |input|
      call('purchase', input).
        concat([{ name: 'dataFields', type: 'object', extends_schema: true,
                  hint: 'Additional fields to be tracked.',
                  properties: call('custom_input_schema', 'input' => input, 'flag' => '', 'object_name' => 'purchase') }])
    end,
    ...
    search_endpoint: lambda do |input|
      ...
      when 'catalogs'
        '/api/catalogs'
      when 'catalog_items'
        "/api/catalogs/#{input['catalogName']}/items"
      when 'lists'
        '/api/lists'
      when 'message_types'
        '/api/messageTypes'
    ...
    search_events_schema: lambda do |_input|
      [
        { name: 'email', label: 'Email address', optional: false, control_type: 'email',
    ...
    search_catalogs_schema: lambda do |_input|
      [
        { name: 'page', hint: 'Page number to list (starting at 1).', sticky: true, control_type: 'integer' },
        { name: 'pageSize', hint: 'Number of results to display per page (defaults to 10).', sticky: true,
          control_type: 'integer', default: '10' }
      ]
    end,
    ...
    sample_get_record: lambda do |input|
      ...
      when 'catalog_item'
        catalog_name = get('/api/catalogs')&.dig('params', 'catalogNames', 0, 'name')
        response = get("/api/catalogs/#{catalog_name}/items")&.dig('params', 'catalogItemsWithProperties', 0)
        result = parse_json(response.to_json)
        call('format_unix_to_utc_time', result)
      end
    end,
    ...
    get_catalog_item_schema: lambda do |input|
      ...
      [
        ...
        if catalog_name.present?
          {
            name: 'itemId',
            label: 'Item ID',
            control_type: 'select',
            pick_list: 'catalog_items',
            pick_list_params: { catalog: 'catalogName' },
            optional: false,
            toggle_hint: 'Select from list',
            toggle_field: {
              name: 'itemId',
              ...
              optional: false,
              toggle_hint: 'Use custom value',
              hint: 'Please enter catalog item ID.'
            }
          }
        end
      ].compact.concat(call('custom_output_schema', ''))
    end,
    get_email_schema: lambda do |_input|
      [
        { name: 'email', label: 'Email address', hint: "User's email", optional: false, control_type: 'email' },
    ...
    delete_endpoint: lambda do |input|
      case input['object_name']
      when 'user'
        "/api/users/#{input['email']}"
      when 'catalog_item'
        "/api/catalogs/#{input['catalogName']}/items/#{input['itemId']}"
      end
    end,
    ...
    delete_catalog_item_schema: lambda do |input|
      catalog_name = input['catalogName'] || ''
      [
        {
          name: 'catalogName',
    ...
    send_schema: lambda do |input|
      [
        ...
        { name: 'dataFields', type: 'object', extends_schema: true,
          hint: 'Fields to merge into email template.',
          properties: call('custom_input_schema', input),
          sticky: true }
      ]
    end,
    ...
    bulk_upsert_object_output: lambda do |input|
      ...
      when 'users'
        [{ name: 'successCount', type: 'integer' },
         ...
         { name: 'invalidUserIds', type: 'array', of: 'string', label: 'Invalid user IDs' }]
      end
    end,
  ...
  object_definitions: {
    ...
    user: {
      fields: lambda do |_connection, _config_fields|
        [
          { name: 'email' },
          { name: 'userId' },
          { name: 'mergeNestedObjects', type: 'object', properties: [] },
          { name: 'mergeArrayKeys', type: 'object', properties: [] },
          { name: 'correlationId' }
        ]
      end
    },
    ...
    event: {
      fields: lambda do |_connection, _config_fields|
        [
          ...
          { name: 'id', sticky: true, label: 'Event ID' },
          {
            sticky: true,
            control_type: 'date_time',
            type: 'date_time',
    ...
    upsert_object: {
      fields: lambda do |_connection, config_fields|
        ...
        when 'event'
          call('upsert_event_schema', config_fields)
        when 'purchase'
          call('upsert_purchase_schema', config_fields)
        end
      end
    },
    ...
    get_object: {
      fields: lambda do |_connection, config_fields|
        ...
        when 'catalog_item'
          call('get_catalog_item_schema', config_fields)
        when 'email'
    ...
    delete_object: {
      fields: lambda do |_connection, config_fields|
        ...
        when 'catalog_item'
          call('delete_catalog_item_schema', config_fields)
        end
      end
    },
  ...
  actions: {
    ...
    update_record: {
      ...
      execute: lambda do |_connection, input|
        ...
        if input['object_name'] == 'user_subscription'
          error('Provide at least one of the following: Email or User ID') if
            input['email'].blank? && input['userId'].blank?
    ...
    upsert_record: {
      ...
      help: lambda do |_input, pick_lists|
        help = {
          ...
          'Event' => 'Track an event. Events are created asynchronously and processed separately from single event ' \
            '(non-bulk) endpoint.',
          'Purchase' => 'Track a purchase event. <b>"shoppingCartItems"</b> field on the user profile is cleared. ' \
            'User profile is also updated if it already exists (created otherwise) using the user request field. '
        }[pick_lists['object_name']] || ''
    ...
    delete_record: {
      ...
      execute: lambda do |_connection, input|
        delete(call('delete_endpoint', input)).
          after_error_response(/.*/) do |_code, body, _header, message|
            error("#{message}: #{body}")
          end
      end,
    ...
    bulk_update: {
      description: lambda do |_input, pick_lists|
        "Bulk update <span class='provider'>#{pick_lists['object_name']&.downcase || 'records'}" \
        "</span> in <span class='provider'>Iterable</span>"\
      end,
      help: lambda do |_input, pick_lists|
        help = {
    ...
    export_data: {
      description: lambda do |input, pick_lists|
        description = case pick_lists['object_name']
                      when 'CSV/JSON'
                        if input['export_format'] == 'CSV'
                          'data to CSV'
                        elsif input['export_format'] == 'JSON'
        ...
        "Export <span class='provider'>#{description || 'data'}" \
        "</span> in <span class='provider'>Iterable</span>"\
      end,
      help: lambda do |_input, pick_lists|
        help = {
    ...
    custom_action: {
      subtitle: 'Build your own Iterable action with a HTTP request',
      ...
      help: {
        body: 'Build your own Iterable action with a HTTP request. ' \
        'The request will be authorized with your Iterable connection.',
        learn_more_url: 'https://api.iterable.com/api/docs',
        learn_more_text: 'Iterable API documentation'
      },
      ...
      config_fields: [
        ...
        },
        {
          ...
          optional: false,
          control_type: 'select',
          pick_list: %w[get post put patch delete].map { |verb| [verb.upcase, verb] }
        }
      ],
  ...
  triggers: {
    new_object: {
      title: 'New object',
      description: lambda do |input, _pick_lists|
        "New <span class='provider'>#{input['eventName']&.labelize&.downcase || 'system'} " \
        "webhook event </span> in <span class='provider'>Iterable</span>"\
      end,
      help: {
        body: "Captures <a href='https://support.iterable.com/hc/en-us/" \
        "articles/208013936-System-Webhooks' target='_blank'>system webhook events</a> from" \
        ' Iterable. Configure Iterable to send webhooks to your Workato <b>' \
        "static webhook URI</b>, click <a href='https://www.workato.com/" \
        "custom_adapters' target='_blank'>here</a> for more details.",
        ...
        [
          {
            ...
            toggle_field: {
              ...
              "en-us/articles/208013936-System-Webhooks#emailOpen' target='_blank'>here</a>."
            }
          }
    ...
    new_workflow: {
      ...
      help: {
        body: "Captures <a href='https://support.iterable.com/hc/en-us/" \
        "articles/205480275-Workflow-Webhooks-' target='_blank'>workflow webhook events</a> from" \
        ' Iterable. Configure Iterable to send webhooks to your Workato <b>' \
        "static webhook URI</b>, click <a href='https://www.workato.com/" \
        "custom_adapters' target='_blank'>here</a> for more details.",
        ...
        [
          {
            ...
            type: 'integer',
            optional: false,
            hint: 'You can find the workflow ID from the browser URL of the' \
  ...
  pick_lists: {
    ...
    catalog_names: lambda do |_connection|
      get('/api/catalogs')&.dig('params', 'catalogNames')&.map { |names| [names['name'], names['name']] }
    end,
    catalog_items: lambda do |_connection, catalog:|
      get("/api/catalogs/#{catalog}/items")&.dig('params', 'catalogItemsWithProperties')&.
        map { |items| [items['itemId'], items['itemId']] }
    end,

{
  ...
  methods: {
    ...
    get_output_fields: lambda do |input|
      url = input[:view_id].present? ? "views/#{input[:view_id]}" : 'users'
      Array.wrap(get(url)&.[]('structure'))
           .reject { |field| field['name'] == 'ID' || %w[image document].include?(field['type']) }.map do |field|
        case field['type']
        when 'number', 'currency', 'percentage', 'autoIncrement', 'relationship'
          { type: 'number' }
        when 'date'
    ...
    get_fields: lambda do |input|
      ...
      structure.map do |field|
        ...
        when 'email'
          ...
          name: "f_#{field['fieldMetaId']}",
          optional: input['id'].present? ? true : !field['required'],
          label: field['name'],
    ...
    format_output: lambda do |res, e_o_s|
      res.each do |key, val|
        res[key] = val.map { |value| { 'choice' => value } } if val.is_a?(Array) && val.first&.is_a?(String)
      end
  ...
  pick_lists: {
    ...
    document_and_image_fields: lambda do |_connection, view_id:|
      document_and_image_fields = Array.wrap(get("views/#{view_id}")&.[]('structure'))
                                       .select { |field| field['type'] == 'document' || field['type'] == 'image' }
                                       .pluck('name', 'name')
      document_and_image_fields.map { |key, value| [key, value.gsub(' ', '%20')] }
    end,
  ...
  actions: {
    ...
    get_all_view_records: {
      ...
      execute: lambda do |_connection, input, _e_i_s, e_o_s|
        url = "views/#{input['view_id']}#{input['query'].present? ? '/find' : ''}"
        response = get(url)
                   .params(max: 1000, q: input['query'])
    ...
    get_file_from_record: {
      ...
      input_fields: lambda do
        [
          ...
          },
          {
            ...
            optional: false,
            hint: <<-HINT
              Select the required record. If the URL is https://go.trackvia.com/#/apps/3/tables/15/views/341/records/view/51/form/584, then <b>51</b> is the record ID.
            HINT
          },
          {
            name: 'field_name',
            type: 'string',
            control_type: 'select',
            pick_list: 'document_and_image_fields',
      ...
      execute: lambda do |_connection, input|
        url = "views/#{input['view_id']}/records/#{input['id']}/files/#{input['field_name']}"
        {
          "content": get(url).headers("Content-Type": 'text/plain').response_format_raw
        }
      end,
    ...
    create_record: {
      ...
      execute: lambda do |_, input, e_i_s, e_o_s|
        ...
        res = post("views/#{input['view_id']}/records")
              .payload(data: Array.wrap(payload))
              .after_error_response(/.*/) do |_code, body, _header, message|
                error("#{message} : #{body}")
              end.dig('data', 0)
        call(:format_output, res, e_o_s)
    ...
    update_record: {
      ...
      execute: lambda do |_connection, input, e_i_s, e_o_s|
        ...
        response = put("views/#{input['view_id']}/records/#{input['id']}")
                   .payload(data: Array.wrap(payload))
                   .after_error_response(/.*/) do |_code, body, _header, message|
                     error("#{message} : #{body}")
                   end&.[]('data')&.first
      ...
      webhook_notification: lambda do |_input, payload, _e_i_s, e_o_s|
        payload.map do |record|
          record.each do |key, val|
            record[key] = val.map { |value| { 'choice' => value } } if val.is_a?(Array) && val.first&.is_a?(String)
          end
          e_o_s.each_with_object({}) do |k, hash|

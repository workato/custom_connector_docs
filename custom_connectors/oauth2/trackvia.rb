{
    ...
    connection: {
        ...
        authorization: {
            ...
            authorization_url: lambda { |connection|
                ...
                "https://#{connection['custom_domain']}/oauth/authorize?" + params
            },

            acquire: lambda { |connection, auth_code, redirect_uri|
                response = post("https://#{connection['custom_domain']}/oauth/token")
                    .payload(
                        client_id: connection['client_id'],
                        client_secret: connection['client_secret'],
                        code: auth_code,
                ...
                user_key = get("https://#{connection['custom_domain']}/3scale/openapiapps")
    ...
    pick_lists: {
        ...
        document_fields: lambda do |_connection, view_id:|
            ...
            select { |field| %w(document image).include?(field['type']) }.pluck('name', 'name')
    ...
    actions: {
        ...
        get_all_view_records: {
            ...
            help: "Fetches records of a specified view in TrackVia. Maximum of #{SEARCH_LIMIT} records will be returned.",
            ...
            execute: lambda do |_connection, input, _e_i_s, e_o_s|
                response = get("views/#{input['view_id']}#{input['query'].present? ? '/find' : ''}").params(max: SEARCH_LIMIT, q: input['query']).
        ...
        upload_file_to_record: {
            ...
            input_fields: lambda do |_object_definitions| [
                ...
                },
                {
                    ...
                    },
                    {
                        ...
                        optional: true,
                        sticky: true,
                        hint: 'Note: File name is mandatory field for JPG and PNG content_type , file name must be give with extention Example: <b> image_file.jpg </b>'
                    }
                ]
            end,
            ...
            execute: lambda do |_connection, input, _e_i_s, e_o_s|
                ...
                payload(file: [input['content'], input['content_type'], input['file_name']].compact).
                after_error_response(/.*/) do |_code, body, _header, message|
                    error("#{message} : #{body}")
                end&.[]('data')
                call(:format_output, res, e_o_s)
        ...
        new_record: {
            ...
            webhook_notification: lambda do |_input, payload, _e_i_s, e_o_s|
                payload.map do |record|
                    ...
                    end.compact.merge(record.slice('id', 'Updated'))
                end
            end,
            ...
            webhook_subscribe: lambda do |webhook_url, connection, input|
                post("zapier/views/#{input['view_id']}/api/hooks",
                    ...
                    event: 'created').merge(view_id: input['view_id']).merge(account_id: connection['account_id'])
            end,

            webhook_unsubscribe: lambda do |webhook|
                delete("zapier/views/#{webhook['view_id']}/api/hooks/#{webhook['id']}").headers('account_id': webhook['account_id'])
            end,

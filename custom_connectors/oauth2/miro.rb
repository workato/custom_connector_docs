...

{
        ...
        [
          ...
          },
          {
            ...
            properties: [
              ...
              {
                name: 'teamAccess',
                label: 'Access within team',
                control_type: 'select',
                pick_list: 'board_access_within_team',
                sticky: true,
  ...
  actions: {
    ...
    create_board: {
      ...
      sample_output: lambda do
        team_id = get('/v1/oauth-token')['team']['id']
        get("/v1/teams/#{team_id}/boards", limit: 1)
          .dig('data', 0) || {}
      end
    },
  ...
  pick_lists: {
    ...
    board_access_within_team: lambda do
      [
    ...
    boards: lambda do
      team_id = get('/v1/oauth-token')['team']['id']
      response =
        get("v1/teams/#{team_id}/boards?fields=id,name&limit=500")
          .after_error_response(/.*/) do |_code, body, _header, message|
          error("#{message}: #{body}")
        end

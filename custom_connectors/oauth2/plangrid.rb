  ...
  object_definitions: {
    ...
    field_report: {
      fields: lambda do |_connection, config_fields|
        ...
        pdf_form_fileds = get("/projects/#{config_fields['project_uid']}/" \
                              'field_reports').params(limit: 1)&.
                          map do |key, _value|
                            { name: key }
                          end&.inject(:merge)
        standard_fields.merge({ 'pdf_form_values' =>
          call('format_schema_field_names', pdf_form_fileds) })
      end
    },
    rfi: {
      fields: lambda do |_connection, _config_fields|
    ...
    new_updated_field_report: {
      ...
      poll: lambda do |_connection, input, closure|
        ...
        results = response['data']&.
          map do |field_report|
            field_report.merge({ 'pdf_form_fileds' =>
              field_report.delete('pdf_form_fileds')&.
                map { |a| { a[0] => a[1] } }&.inject(:merge) }).
              merge('project_uid' => project_uid)
          end
        field_reports = call('format_api_output_field_names', results)
        {

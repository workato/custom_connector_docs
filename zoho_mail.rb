  ...
  methods: {
    ...
    getFolders: lambda do |parsed|
                  ...
                  parsed.each do |folderdata|
                    folder_json << folderdata if folderdata['folderType'] == 'Inbox'
                  end
                  folder_json
                end,
    addContent: lambda do |input|
                  ...
                  response.each do |mailjson|
                    ...
                    content = get("https://mail.#{connection['domain']}/api/"\
                      ...
                      "messages/#{mailjson['messageId']}/content")['data']['content']
  ...
  triggers: {
        ...
        response = get("https://mail.#{connection['domain']}"\
          "/api/accounts/#{account_id}/messages/search")
                   .params(searchKey: 'newMails',
                           ...
                           includeto: 'true',
                           start: offset,
                           receivedTime: Time.now.to_i * 1000 - 3_600_000)['data']
        input = { 'connection' => connection }
                .merge({ 'accountId' => account_id }.merge('response' => response))
    ...
    new_mail_matching_search: {
      ...
      poll: lambda do |connection, input, page|
              ...
              response = get("https://mail.#{connection['domain']}/api/accounts/"\
                "#{account_id}/messages/search")
              ...
              {
                events: response,
                next_page: page > 5 ? nil : response.length >= limit ?
                page + 1 : nil
              }
            end,
              ...
              response = get("https://mail.#{connection['domain']}"\
                "/api/accounts/#{account_id}/messages/search")
                         .params(searchKey: 'in:' + input['folderName'] +
                          '::fromdate:' + (Time.now - 172_800).strftime('%d-%b-%Y'),
                                 limit: limit,
                                 includeto: 'true',
                                 start: offset)['data']
              input = { 'connection' => connection }
